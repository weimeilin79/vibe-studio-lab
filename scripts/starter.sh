...
#   1. the four hands-on files are copied back from starter/
...
for f in stage0_prompt/agent.py stage1_fanout/agent.py stage2_direction/agent.py stage3_router/agent.py agent/graph.py; do
  cp "starter/$f" "$f"
  echo "  restored $f"
done
