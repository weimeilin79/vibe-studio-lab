...
def main() -> int:
    from .server.agent import memory, rag
    missing = []
    if not memory.engine_name():
        missing.append("Memory Bank   -> python -m agent.bank        (or set STUDIO_MEMORY_BANK)")
    if not rag.corpus_name():
        missing.append("RAG corpus    -> python -m agent.rag         (or set STUDIO_RAG_CORPUS)")
    if missing:
